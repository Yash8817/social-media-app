...
def index(request):
    ...
    final_suggesion_list = [x for x in list(new_suggestion_list) if (x not in list(current_user))]
    random.shuffle(final_suggesion_list)

    username_profile = []
    username_profile_list = []
    for user in final_suggesion_list:
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email = email).exists():
                ...
                #log in user and redirect to setting page
...
def settings(request):
    ...
    if request.method == "POST":
        if request.FILES.get("image") == None:
            img = user_profile.profileImage
            user_bio = request.POST['Bio']
            user_location  = request.POST['Location']

            user_profile.profileImage = user_profile.profileImage
...
def profile(request , pk):
    ...
    user_following = len(FollowerCount.objects.filter(followers = pk))
